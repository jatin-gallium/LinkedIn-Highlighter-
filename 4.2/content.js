...
(function () {
  ...
  // V4.1: Known non-post container selectors to exclude
  const ASIDE_SELECTORS = '.scaffold-layout__aside, [data-test-id="aside"], .scaffold-layout__sidebar, .artdeco-card--is-right-rail, .ad-banner-container, .feed-follows-module';

  function isInsideAside(el) {
    return !!el.closest(ASIDE_SELECTORS);
  }
  ...
  function findAllPosts() {
    ...
    // Strategy 4: Walk up from social-counts containers (V4.1: exact class only)
    main
      .querySelectorAll('.social-details-social-counts')
      .forEach((countEl) => {
        if (isInsideAside(countEl)) return;
  ...
  function walkUpToPost(startEl, boundary, maxSteps) {
    ...
    // V4.1: Tighter fallback — require time element (posts always have timestamps)
    // and exclude known non-post containers
    el = startEl;
    for (let i = 0; i < maxSteps && el && el !== boundary && el !== document.body; i++) {
      el = el.parentElement;
      if (!el) break;
  ...
  // V4.1: Validation gate — reject ghost/false-positive post detections
  ...
  function processAllPosts(forceRefresh) {
    ...
    for (const pi of posts) {
      ...
      // V4.1: Reject ghost/false-positive detections
      if (!isValidPost(eng, meta, pi.element)) continue;
  ...
  // ════════════════════════════════════════════════════════════════════
  //  AUTO-SCROLL — v4.1: bidirectional
  // ════════════════════════════════════════════════════════════════════
  ...
  function wireUpSidebarEvents(sidebar) {
    ...
    // V4.1: Shrink/Expand toggle
    $("leh-shrink-toggle").addEventListener("click", () => {
      compactPostCards = !compactPostCards;
  ...
  // ════════════════════════════════════════════════════════════════════
  //  EXTRACT & EXPORT — v4.1: uses data store
  // ════════════════════════════════════════════════════════════════════
